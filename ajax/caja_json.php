...
class Main {
  ...
  public function facturaId($id){
   ...
   public function RegistrarIngresos($factura,$cliente,$valor,$pago){
    $consulta = $this->db->prepare(" INSERT INTO ingresos (no_factura, id_cliente, valor_ingreso, tipo, fecha_ingreso) VALUES (?,?,?,?,now())");
    $consulta->execute([$factura,$cliente,$valor,$pago]);
    $res = $consulta->fetchall();
    return $res;
  }
...
$main->RegistrarIngresos($_POST['no_factura'],$_POST['id_cliente'],$_POST['abono'],$_POST['pago']);
